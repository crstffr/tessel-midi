...
export class SequencerView extends View {
    ...
    stopCapture() {
        ...
        console.log('stop capture', this.state);
        this[this.state]();
    }
    ...
    disableModes() {
        ...
        this.controls.inst.deactivate();
    }

    configMode() {
        this.disableModes();
        this.controls.grid.activate();
        this.controls.inst.activate();
        this.switchInst(this.inst);
    }
    ...
    noteOn(pad, note) {
        switch(this.state) {
            ...
            case 'recordMode':
                pad.load('record');
                let i = this.getSeq().pos;
                this.getSeq().assign(i, note);
                Sequencer.emit('step');
                this.renderSequence();
                Cursor.recordState();
                break;
        }
        this.getInst().playNote(note, this.getStep(pad));
    }
